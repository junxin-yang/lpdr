...
def adaptive_max_pool(input, size):
    # size is (w, h) in this repo; torch.nn.functional expects (H, W)
    out_h, out_w = int(size[1]), int(size[0])
    return F.adaptive_max_pool2d(input, output_size=(out_h, out_w))


def roi_pooling(input, rois, size=(7, 7), spatial_scale=1.0):
    assert (rois.dim() == 2)    # (K,5)
    assert (rois.size(1) == 5)  # [batch_idx, x1, y1, x2, y2]
    output = []
    rois = rois.data.float()
    num_rois = rois.size(0) # number of rois
    ...
    for i in range(num_rois):
        ...
        im = input.narrow(0, im_idx, 1)[..., roi[2]:(roi[4] + 1), roi[1]:(roi[3] + 1)]  # 沿第 0 维（batch 维）从索引 im_idx 开始取长度为 1 的切片
...
def roi_pooling_ims(input, rois, size=(7, 7), spatial_scale=1.0):
    ...
    # size: (w, h)
    assert (rois.dim() == 2)    # (batch_size,4)
    assert len(input) == len(rois)
    assert (rois.size(1) == 4)  # [x1, y1, x2, y2]
    output = []
    rois = rois.data.float()
    num_rois = rois.size(0) # number of rois
